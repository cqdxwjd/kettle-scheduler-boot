...
function getTransErrorList() {
    $('#transerrorList').bootstrapTable({
        url: '/sys/trans/monitor/findErrorRecordList.do',            //请求后台的URL（*）
        method: 'POST',            //请求方式（*）
    ...
    function bindButton() {
        ...
        $('#transerrorList').delegate('#view','click',function(e) {
            ...
            $.ajax({
                type: 'GET',
                async: false,
                url: '/sys/trans/monitor/viewTransRecordDetail.do?transRecordId=' + transRecordId,
                data: {},
                success: function (data) {
                    if (data.success) {
                        layer.open({
        ...
        $('#transerrorList').delegate('#download','click',function(e) {
            ...
            layer.confirm('确定下载该日志记录？', {
                ...
                function (index) {
                    ...
                    form.attr('action', '/sys/trans/monitor/downloadTransRecord.do');
                    var $recordId = $('<input>');
